...
class PopulateExportDamIptuQuitacoes extends Command
{
    /**
     * O nome e a assinatura do comando.
     */
    protected $signature = 'db:populate-export-quitacoes-dams-iptu {--prune : Limpa a tabela antes de popular}';

    /**
     * A descrição do comando.
     */
    protected $description = 'Popula a tabela local export_quitacoes_dams_iptu a partir de payment_entries do PostgreSQL';
    ...
    public function handle()
    {
        ...
        if ($prune) {
            $this->info("Limpando tabela export_quitacoes_dams_iptu...");
            DB::table('export_quitacoes_dams_iptu')->truncate();
        } else {
...
SQL;
        ...
        if ($totalFound === 0) {
            $this->warn("Nenhuma quitação de DAM de IPTU encontrada.");
            return Command::SUCCESS;
        }

        $this->info("Processando {$totalFound} registros...");
        $this->chunkedInsert('export_quitacoes_dams_iptu', $records, $prune);

        $totalInserted = DB::table('export_quitacoes_dams_iptu')->count();
        $this->info("Sucesso! {$totalInserted} registros em export_quitacoes_dams_iptu.");
