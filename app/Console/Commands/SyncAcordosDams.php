...
class SyncAcordosDams extends Command
{
     ...
     */
    protected $description = 'Sincroniza os DAMs de acordos com o Firebird via Stored Procedure';

    /**
     * Stored Procedure principal
     * Nota: O usuário solicitou a "mesma procedure que em SyncQuitacoesDamsAlvaras", 
     * porém para a tabela export_acordos_dams, o procedimento adequado para criação 
     * de DAMs (emissão) é o MIGRACAO_DAMS_1, assim como em SyncDamAlvaras.
     */
    ...
    public function handle()
    {
        ...
        $this->info("Iniciando busca de DAMs em export_acordos_dams no PostgreSQL...");

        $query = DB::table('export_acordos_dams as ed')
        ...
        $results = $query->orderBy('IID_DAM', 'asc')
            ->get();

        $total = count($results);

        if ($total === 0) {
            $this->error("Nenhum DAM pendente encontrado em export_acordos_dams.");
        ...
        foreach ($results as $row) {
            $params = [
                ...
                (string)$row->VDAMNUMERO        // 17. VNUMCODBARRAS DM_VARCHAR_50
            ];
            ...
            try {
                ...
                if ($isSuccess) {
                    ...
                    $sqlError = "SELECT RESULTADO, ID_DAM FROM {$spName}(" . implode(', ', array_map(function ($p) {
        ...
        if (count($failures) > 0) {
            ...
            $this->table(['ID DAM', 'LANÇAMENTO', 'ERRO', 'SQL'], array_map(function ($f) {
                return [$f['id'], $f['lancamento'], substr($f['erro'], 0, 80), substr($f['sql'], 0, 100)];
